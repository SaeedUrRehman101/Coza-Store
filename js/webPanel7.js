...
$(document).ready(function () {
    $('#confirmCode').on('keyup', function () {
        ...
        var invoiceId = $('input[name="invoiceId"]').val(); // Get the hidden invoiceId
        ...
        $.ajax({
            ...
            data: {
                ...
                invoiceId: invoiceId // Send invoiceId with confirmCode
            },
            dataType: 'json',
            success: function (response) {
            ...
            error: function () {
                $('#codeMessage').text('Error while checking code.').addClass('incorrect-message');
            }
        });
    });
});
